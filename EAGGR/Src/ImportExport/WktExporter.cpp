...
namespace EAGGR
{
  ...
  {
    std::string WktExporter::ExportShapes(std::vector<EAGGR::LatLong::LatLongShape> & a_shapes)
    {
          ...
          shapeIndex != a_shapes.size(); shapeIndex++)
      {
        ...
        switch (a_shapes[shapeIndex].GetShapeType())
        {
          ...
          default:
            throw EAGGRException("Unsupported shape in WKT exporter.");
        }
      }
      ...
      // Export the WKT
      char* wktText;
      pGeometry->exportToWkt(&wktText);
      std::string wktString(wktText);
